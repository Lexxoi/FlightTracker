...
function FlightTracker:GetEstimatedFlightTime(origin, destination)
    ...
    -- If a direct recorded time exists, never estimate
    local directKey = origin .. " -> " .. destination
    local reverseKey = destination .. " -> " .. origin
    if FlightTrackerDB.flights[directKey] then return FlightTrackerDB.flights[directKey] end
    if FlightTrackerDB.flights[reverseKey] then return FlightTrackerDB.flights[reverseKey] end
