...
-- Track the last flight master the player was at (used for tooltip + lines)
...
-- Build direct hops using FlightTrackerDB.routes (true taxi connections only)
...
function FlightTracker_MapIcon_OnEnter(button)
    ...
    if not button.nodeName then
        GameTooltip:Show()
        return
    end

    -- Split "Town, Zone" into parts
    ...
    if button.nodeName == origin then
        -- Current location: show recorded route count
        local routes = FlightTrackerDB.routes and FlightTrackerDB.routes[button.nodeName]
        local total  = 0
        local timed  = 0

        if routes then
            for destName in pairs(routes) do
                total = total + 1
                local key        = button.nodeName .. " -> " .. destName
                local reverseKey = destName .. " -> " .. button.nodeName
                if FlightTrackerDB.flights[key] or FlightTrackerDB.flights[reverseKey] then
                    timed = timed + 1
                end
            end
        end

        GameTooltip:AddLine("You are here", 0, 1, 0)
        GameTooltip:AddLine(timed .. " / " .. total .. " flight times recorded", 1, 0.82, 0)

    elseif origin and origin ~= button.nodeName then
        -- Another node: show flight time or estimate from current origin
...
local function RefreshIcons()
    ...
    if zoneNum == 0 then
        -- Continent view: place icons by continent coordinates
        ...
        -- Zone view: place icons by zone coordinates (or city zone if applicable)
...
function MapIcons:RegisterCurrentFlightMaster()
    ...
    ticker:SetScript("OnUpdate", function()
        ...
        if not zoneNum then
            -- Taxi node zone names are sometimes shortened
            -- e.g. "Redridge" vs "Redridge Mountains" — fall back to partial match
        ...
        -- Store current flight master so tooltip/lines know where you are
        FlightTracker.currentFlightMaster = nodeName
        FlightTrackerDB.lastFlightMaster  = nodeName
    end)
end
